-- Active Collaborations Table (FIXED)
-- Tracks confirmed partnerships between churches
-- Fixed: Uses fn_current_user_is_super_admin() instead of profiles.role
...
CREATE TABLE IF NOT EXISTS active_collaborations (
  ...
  started_at TIMESTAMPTZ, -- When both churches confirmed
...
-- Index for efficient lookup by either church
...
-- Helper function to check if user is admin of a church
-- FIXED: Uses fn_current_user_is_super_admin() instead of profiles.role
CREATE OR REPLACE FUNCTION fn_is_church_admin(p_church_id UUID) 
RETURNS BOOLEAN AS $$
BEGIN
  -- Super admins can manage any church (FIXED: use existing helper function)
...
-- Church admins can insert collaborations involving their church
CREATE POLICY "Church admins can create collaborations"
  ON active_collaborations FOR INSERT
  WITH CHECK (
    -- User must be admin of the initiating church
    fn_is_church_admin(initiated_by)
  );

-- Church admins can update collaborations involving their church
CREATE POLICY "Church admins can update their collaborations"
  ON active_collaborations FOR UPDATE
  USING (
    -- User must be admin of either church in the collaboration
    fn_is_church_admin(church_a_id) OR fn_is_church_admin(church_b_id)
  );

-- Super admins can delete collaborations (FIXED: use helper function)
...
-- Function to calculate ministry area overlap between two churches
-- Returns overlap percentage (0-100) based on intersection area
...
BEGIN
  -- Get primary ministry areas for both churches
  ...
  -- If either church has no ministry area, return 0
  ...
  -- Calculate Jaccard-style overlap (intersection over union)
...
-- Function to calculate distance between two churches (in miles)
...
BEGIN
  -- Get locations for both churches
  ...
  -- If either church has no location, return NULL
...
-- Function to get collaboration opportunities with weighted scoring
-- Weights: Area overlap (40%), Callings (25%), Have/Need (20%), Distance (15%)
...
BEGIN
  ...
  WITH partner_data AS (
    ...
    WHERE ch.id != p_church_id
      -- Exclude churches already in active collaboration
      AND NOT EXISTS (
  ...
  final_scores AS (
    SELECT 
      ...
      (
        -- Area overlap: 40% weight, already 0-100
        (sp.overlap_pct * 0.40) +
        -- Shared callings: 25% weight, normalize to 0-100 (assume max 4 callings)
        (LEAST(sp.shared_callings * 25, 100) * 0.25) +
        -- Have/Need matches: 20% weight, normalize to 0-100 (assume max 4 matches)
        (LEAST(sp.collab_matches * 25, 100) * 0.20) +
        -- Distance: 15% weight, inverse relationship (closer = better)
        -- 0 miles = 100 points, 10+ miles = 0 points
        (CASE 
    ...
    -- Filter: If no area overlap, must be within 3 miles
