...
const uploadMiddleware = promisify(upload.single("logo"));

const BUCKET_NAME = "church-logos";
...
export async function POST(req: Request, res: Response) {
  try {
    ...
    if (church?.profile_photo_url) {
      try {
        const oldUrl = church.profile_photo_url;
    ...
    res.json({ profile_photo_url: publicUrl });
  } catch (error: any) {
  ...
  try {
    ...
    if (church?.profile_photo_url) {
      try {
        ...
        console.warn("Failed to cleanup logo:", cleanupError);
      }
    }
