...
export async function POST(req: Request, res: Response) {
  try {
    ...
    if (updateError) {
      console.error('Error approving church:', updateError);
      return res.status(500).json({ error: 'Failed to approve church' });
    }

    // Also set approved=true on the church itself
    await adminClient
      .from('churches')
      .update({ approved: true })
      .eq('id', churchId);

    console.log(`[Admin] Church ${churchId} approved for platform ${platformId} by user ${user.id}`);
    ...
    console.error('Error in POST /api/admin/city-platforms/:id/churches/:churchId/approve:', error);
